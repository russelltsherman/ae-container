...
if [[ ! -s "$claude_state" ]] || ! jq empty "$claude_state" 2>/dev/null; then
  if [[ -s "$claude_seed" ]] && jq empty "$claude_seed" 2>/dev/null; then
    cp "$claude_seed" "$claude_state"
    echo "Seeded ~/.claude.json from host snapshot"
  else
    echo '{}' > "$claude_state"
    echo "warning: host ~/.claude.json snapshot missing/invalid; seeded empty {}" >&2
  fi
fi
